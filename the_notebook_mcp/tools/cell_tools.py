...
class CellToolsProvider:
    ...
    async def notebook_edit_cell(
        self, notebook_path: str, cell_index: int, source: str
    ) -> str:
        ...
        try:
            ...
            logger.exception(
                f"[Tool: notebook_edit_cell] FAILED - Unexpected error: {e}"
            )
            raise RuntimeError(f"An unexpected error occurred: {e}") from e

    async def notebook_add_cell(
        self, notebook_path: str, cell_type: str, source: str, insert_after_index: int
    ) -> str:
        ...
        try:
            ...
            logger.exception(
                f"[Tool: notebook_add_cell] FAILED - Unexpected error: {e}"
            )
            raise RuntimeError(f"An unexpected error occurred: {e}") from e
    ...
    async def notebook_delete_cell(self, notebook_path: str, cell_index: int) -> str:
        ...
        """
        logger.debug(
            f"[Tool: notebook_delete_cell] Called. Args: path={notebook_path}, index={cell_index}"
        )
        try:
            nb = await notebook_ops.read_notebook(
                notebook_path, self.config.allow_root_dirs
            ...
            logger.exception(
                f"[Tool: notebook_delete_cell] FAILED - Unexpected error: {e}"
            )
            raise RuntimeError(f"An unexpected error occurred: {e}") from e

    async def notebook_move_cell(
        self, notebook_path: str, from_index: int, to_index: int
    ) -> str:
        ...
        """
        logger.debug(
            f"[Tool: notebook_move_cell] Called. Args: path={notebook_path}, from={from_index}, to={to_index}"
        )
        try:
            nb = await notebook_ops.read_notebook(
                notebook_path, self.config.allow_root_dirs
            ...
            if not 0 <= from_index < num_cells:
                raise IndexError(
                    f"Source index {from_index} is out of bounds (0-{num_cells - 1})."
                )
            # Allow moving to the very end (index == num_cells)
            if not 0 <= to_index <= num_cells:
                raise IndexError(
                    f"Destination index {to_index} is out of bounds (0-{num_cells})."
                )

            # No change if indices are the same
            if from_index == to_index:
                logger.debug(
                    "[Tool: notebook_move_cell] SKIPPED - Cell move results in no change."
                )
            ...
            logger.exception(
                f"[Tool: notebook_move_cell] FAILED - Unexpected error: {e}"
            )
            raise RuntimeError(f"An unexpected error occurred: {e}") from e

    async def notebook_split_cell(
        self, notebook_path: str, cell_index: int, split_at_line: int
    ) -> str:
        ...
        try:
            ...
            source_part1 = "".join(
                lines[:split_index]
            )  # First part: lines before split point
            source_part2 = "".join(
                lines[split_index:]
            )  # Second part: split point line and all lines after

            # Validate the size of both parts to ensure they don't exceed allowed limits
            max_size = self.config.max_cell_source_size
            if (
                len(source_part1.encode("utf-8")) > max_size
                or len(source_part2.encode("utf-8")) > max_size
            ):
                raise ValueError(
            ...
            if cell_to_split.cell_type == "code":
                new_cell = nbformat.v4.new_code_cell(
                    source=source_part2, metadata=new_cell_metadata
                )
            ...
            logger.exception(
                f"[Tool: notebook_split_cell] FAILED - Unexpected error: {e}"
            )
            raise RuntimeError(
                f"An unexpected error occurred while splitting cell {cell_index}: {e}"
            ) from e

    async def notebook_merge_cells(
        self, notebook_path: str, first_cell_index: int
    ) -> str:
        ...
        """
        logger.debug(
            f"[Tool: notebook_merge_cells] Called. Args: path={notebook_path}, index={first_cell_index}"
        )
        try:
            nb = await notebook_ops.read_notebook(
                notebook_path, self.config.allow_root_dirs
            ...
            if not 0 <= first_cell_index < len(nb.cells) - 1:
                raise IndexError(
                    f"Invalid index {first_cell_index}: Cannot merge last cell or index out of bounds."
                )
            ...
            if cell1.cell_type != cell2.cell_type:
                raise ValueError(
                    f"Cannot merge cells of different types ({cell1.cell_type} and {cell2.cell_type})."
                )
            if cell1.cell_type not in ("code", "markdown"):
                raise ValueError(
                    f"Merging is only supported for code and markdown cells (found {cell1.cell_type})."
                )
            ...
            logger.exception(
                f"[Tool: notebook_merge_cells] FAILED - Unexpected error: {e}"
            )
            raise RuntimeError(
                f"An unexpected error occurred while merging cells: {e}"
            ) from e

    async def notebook_change_cell_type(
        self, notebook_path: str, cell_index: int, new_type: str
    ) -> str:
        ...
        if new_type not in allowed_types:
            raise ValueError(
                f"Invalid target cell type '{new_type}'. Must be one of {allowed_types}."
            )
        ...
        try:
            ...
            if new_type == "code":
                ...
                new_cell = nbformat.v4.new_markdown_cell(
                    source=source, metadata=metadata, attachments=attachments
                )
            else:  # new_type == 'raw'
            ...
            logger.exception(
                f"[Tool: notebook_change_cell_type] FAILED - Unexpected error: {e}"
            )
            raise RuntimeError(
                f"An unexpected error occurred while changing cell type: {e}"
            ) from e

    async def notebook_duplicate_cell(
        self, notebook_path: str, cell_index: int, count: int = 1
    ) -> str:
        ...
        try:
            ...
            logger.exception(
                f"[Tool: notebook_duplicate_cell] FAILED - Unexpected error: {e}"
            )
            raise RuntimeError(
                f"An unexpected error occurred while duplicating cells: {e}"
            ) from e
