...
class DiagnosticToolsProvider:
    ...
    async def diagnose_imports(self) -> str:
        ...
        for lib_name, description in libraries_to_check:
            try:
                ...
                if spec and spec.loader:
                    ...
                    logger.trace(
                        f"[Tool: diagnose_imports] Found {lib_name} v{version} at {path}"
                    )
                else:
                ...
                results.append(
                    f"  [FAIL] {lib_name}: Import error '{e}' - {description}"
                )
                logger.error(
                    f"[Tool: diagnose_imports] Import error for {lib_name}: {e}"
                )
                status = "FAIL"
            except Exception as e:
                results.append(
                    f"  [ERROR] {lib_name}: Unexpected error during check '{e}' - {description}"
                )
                logger.exception(
                    f"[Tool: diagnose_imports] Unexpected error checking {lib_name}: {e}"
                )
                status = "ERROR"
        ...
        logger.info(
            f"[Tool: diagnose_imports] SUCCESS - Diagnostics complete. Status: {status}",
            tool_success=True,
        )
        return summary
