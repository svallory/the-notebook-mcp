...
class FileToolsProvider:
    ...
    async def notebook_create(self, notebook_path: str) -> str:
        ...
        except (PermissionError, FileExistsError, ValueError, IOError, nbformat.validator.ValidationError) as e:
    ...
    async def notebook_delete(self, notebook_path: str) -> str:
        ...
        try:
            ...
            logger.info(f"[Tool: notebook_delete] SUCCESS - Deleted notebook at {resolved_path}", tool_success=True)
    ...
    async def notebook_rename(self, old_path: str, new_path: str) -> str:
        ...
        try:
            ...
            logger.info(f"[Tool: notebook_rename] SUCCESS - Renamed notebook from {resolved_old_path} to {resolved_new_path}", tool_success=True)
    ...
    async def notebook_export(
        ...
        """
        logger.debug(f"[Tool: notebook_export] Called. Args: source={notebook_path}, format={export_format}, output={output_path}")
        ...
        try:
            ...
            logger.info(f"[Tool: notebook_export] SUCCESS - Exported notebook to {resolved_output_path}", tool_success=True)
            return f"Successfully exported notebook to {output_path}"

        except (ValueError, PermissionError, FileNotFoundError, OSError, IOError, RuntimeError, nbformat.validator.ValidationError) as e:
